...
void onComandoRecibido(const uint8_t* mac, const uint8_t* data, int len) {
    ...
    case CMD_START_TRIP:
        ...
        modoManual = false; joySteer = 0;
        pausandoEn = 0;
        navState = GOING_CEBO1; histCount = 0; totalDist = 0;
        maxSpeed = 0; sumSpeed = 0; speedSamples = 0;
        lastLat = smoothLat; lastLng = smoothLng;
        tripStartMs = millis();
        cebo1Disparado = false; cebo2Disparado = false;
        setCebo1(false); setCebo2(false);
        targetBearing = calcBearing(smoothLat, smoothLng, cebo1.lat, cebo1.lng);
        motorRunning = true; setMotorPct(throttleMax);
        break;
...
void SetupESPNowBarco() {
    ...
    Serial.print("MAC Barco: "); Serial.println(WiFi.macAddress());
}
...
void EnviarTelemetria() {
    ...
    t.homeLat = (float)home.lat;  t.homeLng = (float)home.lng;  t.homeValid = home.valid;
    t.c1Lat   = (float)cebo1.lat; t.c1Lng   = (float)cebo1.lng; t.c1Valid   = cebo1.valid;
    t.c2Lat   = (float)cebo2.lat; t.c2Lng   = (float)cebo2.lng; t.c2Valid   = cebo2.valid;
