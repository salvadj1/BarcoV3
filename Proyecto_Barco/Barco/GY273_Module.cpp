...
// ---------- SPIFFS ----------
struct CalibData { float oX, oY, oZ, sX, sY; };
...
static bool loadCalib() {
    ...
    offsetX = d.oX; offsetY = d.oY; offsetZ = d.oZ;
    scaleX  = d.sX; scaleY  = d.sY;
    gy273OffsetX = offsetX; gy273OffsetY = offsetY;
    gy273ScaleX  = scaleX;  gy273ScaleY  = scaleY;
...
static void saveCalib() {
    ...
    if (!f) { Serial.println("GY273: error guardando calib"); return; }
...
void SetupGY273() {
    ...
    Wire.write(0x00); Wire.write(0x78);  // CRA: 8avg, 75Hz, normal
    Wire.endTransmission();
    delay(5);
    Wire.beginTransmission(0x1E);
    Wire.write(0x01); Wire.write(0x20);  // CRB: gain 1
    Wire.endTransmission();
    delay(5);
    Wire.beginTransmission(0x1E);
    Wire.write(0x02); Wire.write(0x00);  // MODE: continuo
    Wire.endTransmission();
    delay(15);
...
bool CalibracionGY273Completa() { return calibCompleta; }
